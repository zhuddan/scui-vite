  ...
  methods: {
    ...
    menuFilter(queryString) {
      ...
      const filterMenu = this.menu.filter((item) => {
        if ((item.meta.title).toLowerCase().includes(queryString.toLowerCase())) {
          return true
        }
...
<template>
  <div class="sc-search">
    ...
    <div v-if="history.length > 0" class="sc-search-history">
      <el-tag v-for="(item, index) in history" :key="item" closable effect="dark" type="info" @click="historyClick(item)" @close="historyClose(index)">
        {{ item }}
      </el-tag>
    </div>
