      ...
      <el-card v-for="task in tasks" :key="task.id" shadow="hover" class="user-bar-tasks-item">
        <div class="user-bar-tasks-item-body">
          ...
          <div class="taskMain">
            <div class="title">
              <h2>{{ task.taskName }}</h2>
              <p><span v-time.tip="task.createDate" /> 创建</p>
            </div>
            <div class="bottom">
              <div class="state">
                ...
                <el-tag v-if="task.state == '1'">
                  完成
                </el-tag>
              </div>
              <div class="handler">
                <el-button v-if="task.state == '1'" type="primary" circle icon="el-icon-download" @click="download(task)" />
              </div>
            </div>
          </div>
        </div>
