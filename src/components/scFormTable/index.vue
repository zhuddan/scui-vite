  ...
  <div ref="scFormTable" class="sc-form-table">
    <el-table ref="table" :data="data" border stripe>
      <el-table-column type="index" width="50" fixed="left">
        <template #header>
          <el-button v-if="!hideAdd" type="primary" icon="el-icon-plus" size="small" circle @click="rowAdd" />
        </template>
        <template #default="scope">
          <div class="sc-form-table-handle" :class="[{ 'sc-form-table-handle-delete': !hideDelete }]">
            <span>{{ scope.$index + 1 }}</span>
            <el-button v-if="!hideDelete" type="danger" icon="el-icon-delete" size="small" plain circle @click="rowDel(scope.row, scope.$index)" />
          </div>
        </template>
      </el-table-column>
