  ...
  <div class="sc-file-select">
    ...
    <div v-loading="listLoading" class="sc-file-select__files">
      ...
      <div class="sc-file-select__pagination">
        <el-pagination v-model:current-page="currentPage" small background layout="prev, pager, next" :total="total" :page-size="pageSize" @current-change="reload" />
      </div>
      <div class="sc-file-select__do">
        <slot name="do" />
        <el-button type="primary" :disabled="value.length <= 0" @click="submit">
          确 定
        </el-button>
      </div>
    </div>
