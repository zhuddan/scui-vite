...
<template>
  <div class="sc-filterBar">
    <slot :filter-length="filterObjLength" :open-filter="openFilter">
    ...
    <el-drawer v-model="drawer" title="过滤器" :size="650" append-to-body>
      <el-container v-loading="saveLoading">
        <el-main style="padding:0">
          <el-tabs class="root">
                ...
                <div class="sc-filter-main">
                  ...
                  <table v-else>
                    ...
                    <tr v-for="(item, index) in filter" :key="index">
                      ...
                      <td>
                        <PySelect v-model="item.field" :options="fields" :filter="filter" placeholder="过滤字段" filterable @change="fieldChange(item)" />
                      </td>
                      ...
                      <td>
                        ...
                        <!-- 自定义日期 -->
                        <el-date-picker v-if="item.field.type == 'customDate'" v-model="item.value" :type="item.field.extend.dateType || 'date'" :value-format="item.field.extend.valueFormat" :placeholder="item.field.placeholder || '请选择'" start-placeholder="开始日期" end-placeholder="结束日期" style="width: 100%;" />
                        <!-- 开关 -->
                  ...
                  <el-button type="primary" text icon="el-icon-plus" @click="addFilter">
                    增加过滤项
                  </el-button>
                </div>
              </el-scrollbar>
            ...
            <el-tab-pane lazy>
              ...
              <el-scrollbar>
                <My ref="my" :data="myFilter" :filter-name="filterName" @select-myfilter="selectMyfilter" />
              </el-scrollbar>
            </el-tab-pane>
          </el-tabs>
        </el-main>
        <el-footer>
          ...
          <el-button type="primary" plain :disabled="filter.length <= 0" @click="saveMy">
            另存为常用
          </el-button>
