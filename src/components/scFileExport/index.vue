...
export default {
  ...
  watch: {
    'formData.fileType': function (val) {
      ...
      apiObj.get(data, {
        ...
        onDownloadProgress(e) {
          if (e.lengthComputable) {
            _this.downLoadProgress = Number.parseInt(e.loaded / e.total * 100)
          }
        },
      }).then((res) => {
      ...
      <div v-loading="downLoading" element-loading-text="正在处理中...">
        ...
        <el-tabs>
          <el-tab-pane label="常规" lazy>
            ...
            <el-button v-if="async" type="primary" size="large" icon="el-icon-plus" style="width: 100%;" :loading="asyncLoading" @click="download">
              发起导出任务
            </el-button>
            <el-button v-else type="primary" size="large" icon="el-icon-download" style="width: 100%;" @click="download">
              下 载
            </el-button>
          </el-tab-pane>
          <el-tab-pane v-if="columnData.length > 0" label="列设置" lazy>
            <ColumnSet :column="columnData" />
          </el-tab-pane>
