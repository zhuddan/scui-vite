...
export default {
  ...
  props: {
    ...
    aspectRatio: { type: Number, default: Number.NaN },
  },
  data() {
    return {
      value: '',
  ...
  methods: {
    ...
    request(param) {
      ...
      apiObj.post(data, {
        onUploadProgress: (e) => {
          const complete = Number.parseInt(((e.loaded / e.total) * 100) | 0, 10)
...
<template>
  <div class="sc-upload" :class="{ 'sc-upload-round': round }" :style="style">
    ...
    <el-upload
      v-if="!file" ref="uploader" class="uploader"
    ...
    <el-dialog v-model="cropperDialogVisible" title="剪裁" draggable :width="580" destroy-on-close @closed="cropperClosed">
      <ScCropper ref="cropper" :src="cropperFile.tempCropperFile" :compress="compress" :aspect-ratio="aspectRatio" />
      <template #footer>
        <el-button @click="cropperDialogVisible = false">
          取 消
        </el-button>
        <el-button type="primary" @click="cropperSave">
          确 定
        </el-button>
      </template>
    </el-dialog>
