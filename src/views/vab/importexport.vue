    ...
    <el-row :gutter="15">
      <el-col :lg="12">
        <el-card shadow="never" header="导入(使用mock,有50%几率导入失败)">
          <ScFileImport :api-obj="$API.common.importFile" template-url="http://www.scuiadmin/file.xlsx" @success="success" />
          <ScFileImport :api-obj="$API.common.importFile" :data="{ otherData: 'demo' }" template-url="http://www.scuiadmin/file.xlsx" accept=".xls, .xlsx" :max-size="30" tip="请上传小于或等于 30M 的 .xls, .xlsx 格式文件(自定义TIP)" @success="success">
            <template #default="{ open }">
              <el-button type="primary" icon="sc-icon-upload" @click="open">
                导入(全配置)
              </el-button>
            </template>
            <template #uploader>
          ...
          <el-descriptions :column="1" border size="small" style="margin-top: 15px;">
            <el-descriptions-item label="apiObj" :width="200">
              Object 文件上传接口对象
      ...
      <el-col :lg="12">
        <el-card shadow="never" header="导出">
          <ScFileExport :api-obj="$API.common.exportFile" />
          <ScFileExport :api-obj="$API.common.exportFile" file-name="人员列表(异步)" async>
            <template #default="{ open }">
              <el-button type="primary" icon="sc-icon-download" @click="open">
                导出(异步)
              </el-button>
            </template>
          </ScFileExport>
          <ScFileExport :api-obj="$API.common.exportFile" blob file-name="人员列表" :data="{ otherData: 'demo' }" show-data :column="column" :file-types="['xlsx', 'docx', 'pdf']">
            <template #default="{ open }">
              <el-button type="primary" icon="sc-icon-download" @click="open">
                导出(blob文件流)
              </el-button>
    ...
    <div style="margin-top: 15px;">
      <el-table :data="importErrData.failList" border stripe max-height="270" style="width: 100%">
        ...
        <el-table-column prop="" label="状态" width="100">
          <el-tag type="danger">
            <el-icon><el-icon-circle-close-filled /></el-icon> 失败
          </el-tag>
        </el-table-column>
