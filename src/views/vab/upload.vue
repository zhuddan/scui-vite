...
<template>
  <el-main>
    ...
    <el-card shadow="never" header="图片卡片示例(已开启拖拽排序)">
      <sc-upload-multiple v-model="fileurl2" draggable :limit="3" tip="最多上传3个文件,单个文件不要超过10M,请上传图像格式文件" />
    </el-card>

    <el-card shadow="never" header="单图像示例">
      <el-space wrap :size="8">
        <sc-upload v-model="fileurl3" />
        <sc-upload v-model="fileurl4" title="自定义标题" icon="el-icon-picture" />
        <sc-upload v-model="fileurl5" :api-obj="uploadApi" accept="image/jpg,image/png" :on-success="success" :width="220">
          <div class="custom-empty">
            <el-icon><el-icon-upload /></el-icon>
            <p>自定义插槽</p>
          </div>
        </sc-upload>
        <sc-upload v-model="fileurl6" round icon="el-icon-avatar" title="开启圆形" />
        <sc-upload v-model="fileurl7" title="开启剪裁" :cropper="true" :compress="1" :aspect-ratio="1 / 1" />
      </el-space>
    </el-card>
        ...
        <el-form-item label="身份证" required>
          <el-space wrap :size="8">
            <el-form-item prop="file1">
              <sc-upload v-model="form.file1" title="人像面" />
            </el-form-item>
            <el-form-item prop="file2">
              <sc-upload v-model="form.file2" title="国徽面" />
            </el-form-item>
          </el-space>
        </el-form-item>
        <el-form-item label="其他凭证" prop="file3">
          <sc-upload-multiple v-model="form.file3" />
        </el-form-item>
