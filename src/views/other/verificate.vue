        ...
        async: [
          {
            ...
            validator: (rule, value, callback) => {
              this.$API.demo.ver.get({ value }).then((res) => {
                if (res.data != value) {
    ...
    <el-card shadow="never" header="async-validator内置">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px" status-icon>
        <el-form-item label="必填" prop="required">
          <el-input v-model="form.required" />
        </el-form-item>
        <el-form-item label="长度" prop="length">
          <el-input v-model="form.length" placeholder="长度需为3位" />
        </el-form-item>
        <el-form-item label="类型" prop="type">
          <el-input v-model="form.type" placeholder="类型可为string number boolean array date url email等" />
        </el-form-item>
        <el-form-item label="范围" prop="range">
          <el-input v-model="form.range" placeholder="请填写范围在3至5位" />
        </el-form-item>
        <el-form-item label="枚举" prop="enum">
          <el-input v-model="form.enum" placeholder="请填写admin，user，guest其一" />
        </el-form-item>
        <el-form-item label="自定义" prop="custom">
          <el-input v-model="form.custom" placeholder="请填写数字1" />
        </el-form-item>
        <el-form-item label="异步验证" prop="async">
          <el-input v-model="form.async" placeholder="请输入SCUI最新版本号,form开启status-icon后 可以查看右侧状态" />
        </el-form-item>
        <el-form-item>
