...
<template>
  <el-container>
    <el-aside v-loading="showDicloading" width="300px">
      <el-container>
        <el-header>
          <el-input v-model="dicFilterText" placeholder="输入关键字进行过滤" clearable />
        </el-header>
          ...
          <el-tree ref="dic" class="menu" node-key="id" :data="dicList" :props="dicProps" :highlight-current="true" :expand-on-click-node="false" :filter-node-method="dicFilterNode" @node-click="dicClick">
            <template #default="{ node, data }">
              <span class="custom-tree-node">
                ...
                <span class="do">
                  <el-button-group>
                    <el-button icon="el-icon-edit" size="small" @click.stop="dicEdit(data)" />
                    <el-button icon="el-icon-delete" size="small" @click.stop="dicDel(node, data)" />
                  </el-button-group>
                </span>
              </span>
            </template>
    ...
    <el-container class="is-vertical">
      ...
      <el-main class="nopadding">
        <scTable ref="table" :api-obj="listApi" row-key="id" :params="listApiParams" stripe pagination-layout="prev, pager, next" @selection-change="selectionChange">
          ...
          <el-table-column label="是否有效" prop="yx" width="100">
            <template #default="scope">
              <el-switch v-model="scope.row.yx" :loading="scope.row.$switch_yx" active-value="1" inactive-value="0" @change="changeSwitch($event, scope.row)" />
            </template>
          </el-table-column>
  ...
  <DicDialog v-if="dialog.dic" ref="dicDialog" @success="handleDicSuccess" @closed="dialog.dic = false" />

  <ListDialog v-if="dialog.list" ref="listDialog" @success="handleListSuccess" @closed="dialog.list = false" />
</template>
