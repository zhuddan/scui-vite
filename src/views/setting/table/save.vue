...
<template>
  <el-drawer v-model="visible" :title="titleMap[mode]" :size="1000" destroy-on-close @closed="$emit('closed')">
    <el-container v-loading="loading">
      <el-main style="padding:0 20px 20px 20px">
        <el-form ref="dialogForm" :model="form" :rules="rules" label-width="100px" label-position="top">
          ...
          <el-row :gutter="20">
            <el-col :span="16">
              <el-form-item label="表格名称" prop="name">
                <el-input v-model="form.name" placeholder="请输入中文描述标题" />
              </el-form-item>
            </el-col>
            <el-col :span="8">
          ...
          <el-form-item>
            <el-checkbox v-model="form.remoteSort" label="远程排序" />
            <el-checkbox v-model="form.remoteFilter" label="远程过滤" />
          </el-form-item>
          <sc-title title="表格列" />
          <el-form-item prop="column">
            <sc-form-table v-model="form.column" :add-template="addTemplate" drag-sort placeholder="暂无数据">
              ...
              <el-table-column prop="prop" label="字段" width="150">
                <template #default="scope">
                  <el-input v-model="scope.row.prop" placeholder="请输入字段" />
                </template>
              </el-table-column>
              ...
              <el-table-column prop="filters" label="过滤项" width="90" align="center">
                <template #default="scope">
                  <el-button text size="small" :type="scope.row.filters.length == 0 ? '' : 'primary'" @click="setFilters(scope.row.filters)">
                    过滤项
                  </el-button>
                </template>
              </el-table-column>
    ...
    <el-drawer v-model="setFiltersVisible" title="过滤项配置" :size="500" destroy-on-close>
      <el-main style="padding:0 20px 20px 20px">
        <sc-form-table v-model="selectionFilters" :add-template="filtersAddTemplate" drag-sort placeholder="暂无数据">
          <el-table-column prop="text" label="名称">
            <template #default="scope">
              <el-input v-model="scope.row.text" placeholder="请输入名称" />
            </template>
          </el-table-column>
