...
export default {
  ...
  methods: {
    ...
    async getMenu() {
      ...
      this.$nextTick(() => {
        const filterKeys = this.menu.checked.filter(key => this.$refs.menu.getNode(key).isLeaf)
    ...
    async getDept() {
      ...
      this.$nextTick(() => {
        const filterKeys = this.data.checked.filter(key => this.$refs.dept.getNode(key).isLeaf)
        ...
        <el-form label-width="100px" label-position="left">
          <el-form-item label="规则类型">
            <el-select v-model="data.dataType" placeholder="请选择">
              ...
              <el-option label="本人可见" value="2" />
              <el-option label="所在部门可见" value="3" />
              <el-option label="所在部门及子级可见" value="4" />
              <el-option label="选择的部门可见" value="5" />
          ...
          <el-form-item v-show="data.dataType == '6'" label="规则值">
            <el-input v-model="data.rule" clearable type="textarea" :rows="6" placeholder="请输入自定义规则代码" />
          </el-form-item>
        </el-form>
      </el-tab-pane>
      <el-tab-pane label="控制台模块">
        <div class="treeMain">
          <el-tree ref="grid" node-key="key" :data="grid.list" :props="grid.props" :default-checked-keys="grid.checked" show-checkbox />
        </div>
      </el-tab-pane>
