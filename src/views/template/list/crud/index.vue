...
<template>
  <el-container>
    ...
    <el-main class="nopadding">
      <scTable ref="table" :api-obj="list.apiObj" row-key="id" stripe @selection-change="selectionChange">
        ...
        <el-table-column label="状态" prop="boolean" width="60">
          <template #default="scope">
            <sc-status-indicator v-if="scope.row.boolean" type="success" />
            <sc-status-indicator v-if="!scope.row.boolean" pulse type="danger" />
          </template>
        </el-table-column>
        <el-table-column label="评分" prop="num" width="150" />
        <el-table-column label="操作" fixed="right" align="right" width="300">
          <template #default="scope">
            <el-button plain size="small" @click="table_show(scope.row)">
              查看
            </el-button>
            <el-button type="primary" plain size="small" @click="table_edit(scope.row)">
              编辑
            </el-button>
            <el-button type="primary" plain size="small" @click="table_edit_page(scope.row)">
              页面编辑
            </el-button>
