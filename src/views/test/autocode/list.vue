...
<template>
  <el-container>
      ...
      <el-card shadow="never">
        <el-tabs tab-position="top">
          <el-tab-pane label="列配置">
            <sc-form-table v-model="column" :add-template="addTemplate" placeholder="请添加列数据">
              ...
              <el-table-column prop="prop" label="字段名" width="180">
                <template #default="scope">
                  <el-input v-model="scope.row.prop" placeholder="请输入内容" />
                </template>
              </el-table-column>
              <el-table-column prop="width" label="宽度" width="180">
                <template #default="scope">
                  <el-input v-model="scope.row.width" placeholder="请输入内容" />
                </template>
              </el-table-column>
          ...
          <el-tab-pane label="API路径配置">
            <el-alert title="$API 映射文件: @/api/index.js 统一接口管理器, 所以需提前配置好API对象." type="warning" style="margin:0 0 20px 0;" />
            <el-row>
              <el-col :xl="12" :lg="8">
    ...
    <el-footer>
      <el-dropdown style="margin-right: 15px;">
        <el-button type="primary" icon="el-icon-download" :loading="downloadcodeLoading">
          下载VUE文件
        </el-button>
        <template #dropdown>
          <el-dropdown-menu>
      ...
      <el-dropdown>
        <el-button type="primary" plain icon="el-icon-top-right" :loading="showcodeLoading">
          预览代码
        </el-button>
        <template #dropdown>
          <el-dropdown-menu>
  ...
  <el-dialog v-model="codeVisible" title="代码预览" width="60%" append-to-body destroy-on-close>
    <el-alert title="需将VUE文件放置views文件夹,路由匹配组件的路径下,如文件名为index.vue可不需要写文件名" type="success" show-icon style="margin-bottom: 20px;" />
